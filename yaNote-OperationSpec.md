...
- **各ボタンの機能:**
  ...
  - **エクスポートボタン（`保存`）**
    ...
    - 保存データには、ノードおよび線の情報、線種、ノードの太字設定、キャンバスのパン状態、バージョン情報が含まれます。
...
- **自動保存:**
  - 各操作完了時に、現在の状態（ノード、線、線種、ノードの太字設定、キャンバスのパン状態）がローカルストレージに保存されます。
- **多タブ同期:**
...
- **設定変更時の確認:**
  - ボタンクリック時に、変更内容を示すツールチップが一時的に表示されます（例：「太字: オン」）。
...
### 10.1. パン移動の改善
- **機能概要:**
  - マウスホイールやトラックパッドでの直感的なパン操作を追加
  - スクロールバーを非表示にして、キャンバス表示領域を拡大
  - 様々なデバイスでの操作性向上

- **操作方法:**
  - マウスホイール回転: 縦方向のパン移動
  - Shiftキー+マウスホイール: 横方向のパン移動
  - トラックパッド二本指スクロール: 任意方向へのパン移動

### 10.2. ノードテキスト改行機能
- **機能概要:**
  - Shift+Enterキーによるノードテキストの改行に対応
  - 複数行テキストでもノードのサイズが適切に調整
  - 改行を含むテキストの編集と表示を最適化

- **視覚的フィードバック:**
  - 改行時にノードの高さが自動調整される
  - 複数行テキストでも接続線がノードの中心と適切に連携
...
この仕様書により、ユーザーは直感的な操作（ノードや線の生成・編集・移動、太字設定、線種変更、グループ選択、パン操作、Undo／Redo）を通じて、柔軟かつ効率的に yaNote を利用できます。