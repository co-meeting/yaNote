...
### 1.4. 囲み矩形（v1.7）
- **概要**:  
  ノードたちを視覚的にかこうための角丸・薄背景の矩形。常にノード・接続線より背面に表示されます。  
  ノードとの所属関係（グループ）は持たない純粋な描画要素で、矩形を移動しても中のノードは動きません。

- **描画**:  
  空白部分での **Shift＋左ドラッグ**、またはコントロールパネルの「▭」ボタンでモードをオンにしてからドラッグ（タッチは後者のみ）。  
  描き終わるとモードは自動解除されます（ワンショット）。モードは「▭」再クリックまたは Escape でも解除できます。  
  ドラッグが約40px（`ENCLOSURE_MIN_SIZE`）未満の場合はクリック誤爆とみなし、矩形は作成されません。

- **選択・移動・リサイズ・削除**:  
  枠線（外周±8px、`ENCLOSURE_HIT_MARGIN`）のクリック／タップで選択され、四隅にハンドルが表示されます。  
  枠線のドラッグで移動、四隅ハンドルのドラッグで対角を固定したままリサイズ（最小サイズあり）、Delete / Backspace で削除します。  
  矩形の内部は通常の空白として扱われ（`pointer-events: none`）、内部でのノード生成・ラバーバンド選択・パンを阻害しません。
...
- **ラバーバンド選択**:  
  空白部分で **左クリック＋ドラッグ** すると、矩形（ラバーバンド）が描画され、矩形内または矩形と交差するすべてのノードおよび接続線が一括で選択されます。  
  囲み矩形は、ラバーバンドで**完全に囲んだ場合のみ**選択に含まれます（交差だけでは選択されません。矩形内部でのラバーバンド選択に巻き込まれないため）.
...
- **ズーム（拡大縮小）**:  
  Ctrl/Cmd＋マウスホイール（トラックパッドのピンチ操作を含む）で、カーソル位置を中心に表示を拡大縮小します。  
  Ctrl/Cmd+0 を押すと、画面中央を基準に等倍（100%）へ戻ります。

- **ズームの技術的な仕様**:
  - ズーム範囲は 25%〜200%（`app.js` の `ZOOM_MIN`／`ZOOM_MAX` 定数で一元管理）。
  - ズーム中はアンカー（カーソル位置または指の中間点）直下の点が画面上で動かないよう、パン位置が自動補正されます。
  - macOS Safari のトラックパッドピンチは wheel イベントではなく gesture イベント（gesturestart/gesturechange）経由で処理されます。
  - ズーム操作中は画面右下に現在のズーム率が約1.5秒間表示されます。
  - ズーム率は状態の一部として保存され（localStorage・エクスポートJSON・共有URL）、復元時に適用されます。ズーム率を持たない旧形式データは 100% として読み込まれます。
  - ズーム操作自体はパンと同様に Undo 履歴には追加されません（次の編集操作の保存に含まれます）。
...
- **自動リセット**:  
  ...
  - 利用ガイド表示（「？」ボタン）を実行した場合
  - 共有URLからノートを読み込んだ場合
  - ※ ファイルメニュー（☰）の「保存したノートを開く」からJSONファイルを読み込んだ場合は、元のパン位置を維持します。
...
## 6. Undo／Redo と状態管理
- **自動保存**:  
  各操作（ノード・接続線・囲み矩形の生成、編集、移動、削除）が完了するたびに、現在の状態が自動的に保存されます.  
  囲み矩形は状態データの `enclosures` 配列（`{id, x, y, w, h}`）として保存され、localStorage・JSONエクスポート/インポート・URL共有のすべてに含まれます。`enclosures` を持たない旧形式のデータも空配列として問題なく読み込めます.
...
- **エクスポート／インポート**:  
  - **エクスポート**: 現在の状態が JSON 形式でファイル出力され、ファイル名にはタイトルと日時が自動付与されます.  
    - JSON のトップレベルには `semantics`（書き手のノード種別・接続線の使い分けの傾向を説明する凡例テキスト）が埋め込まれます。インポート時には無視される write-only のフィールドです.
  - **AI用エクスポート**: ファイルメニュー（☰）の「AI用Markdownで保存」により、現在のノートを AI 向け Markdown（`.md`）としてファイル出力します。ファイル名の形式はエクスポートと同じです.  
    - Markdown には、凡例に加えて、座標配置と接続トポロジーから解析した「トピックの島」「カラム対応（text-only太字の見出しに揃った列）」「接続の階層（実線矢印＝親子、破線＝注釈）」がアウトライン形式で出力されます。解析のしきい値は `app.js` の `AI_EXPORT` 定数で調整できます.
  - **インポート**: 保存済みの JSON ファイルから状態が復元され、内部履歴に追加されます（Markdown はインポート対象外）.
...
- **グループ構成**:  
  ボタンは縦線のセパレータで3グループに区切られています。
  1. **編集系**: 太字（B）、整列（▦）、ノード種類変更、線種変更、線タイプ変更、囲み矩形（▭）
  2. **表示系**: 表示位置のリセット（⊙）
  3. **ファイル系**: ファイルメニュー（☰）、ヘルプ（？）
...
- **主な機能**:
  ...
  - **囲み矩形（▭）**:  
    囲み矩形の描画モードをトグルします。モード中は次の1ドラッグ（マウス・タッチ共通）が矩形の描画になり、描き終わると自動解除されます.
  - **整列**:  
...
- **ファイルメニュー（☰）**:  
  「☰」ボタンをクリックすると、ファイル操作のドロップダウンメニューが開きます。項目の選択、メニュー外のクリック（タップ）、Escape キーのいずれかで閉じます。
  - **新規ノートを作成**:  
  ...
  - **URLで共有**:  
    共有モーダルを表示します（詳細は「10. URL共有機能」を参照）.
...
- **技術的な仕様**:
  ...
  - 右端のボタン（「？」や「☰」など）の場合、ツールチップが画面外にはみ出さないよう左寄りに表示されます。
...
| Shift＋ドラッグ             | 空白                   | Shiftキーを押しながら左クリック＋ドラッグ（「▭」ボタン後のドラッグでも同じ） | 囲み矩形が最背面に描かれる.                             |
| 左クリック＋ドラッグ         | 囲み矩形の枠線          | 枠線をクリックで選択、枠線をドラッグで移動、四隅ハンドルでリサイズ | 矩形が選択・移動・リサイズされる（中のノードは動かない）.   |
| 右クリック＋ドラッグ         | キャンバス             | 右クリック＋ドラッグ                                               | キャンバス全体が移動（パン）される.                    |
| Ctrl/Cmd＋ホイール          | キャンバス             | Ctrl/Cmdキーを押しながらマウスホイール（トラックパッドのピンチ含む） | カーソル位置を中心に表示が拡大縮小される（25%〜200%）.      |
| Ctrl/Cmd+0                 | キャンバス             | キーボードショートカット Ctrl/Cmd+0                               | 表示が等倍（100%）に戻る.                              |
...
| Cmd/Ctrl+A (全オブジェクト選択) | 全オブジェクト       | キーボードショートカット Cmd/Ctrl+A                               | 全ノード・接続線・囲み矩形が一括で選択される。ただし、ノードのテキスト編集中の場合はブラウザの全選択動作が優先される. |
...
### 10.2. 共有機能の操作
- **共有リンク生成**:  
  - ファイルメニュー（☰）の「URLで共有」を選択すると、共有モーダルが表示されます。
...
- **動作**:  
  ...
  - ファイルメニューの「新規ノートを作成」を実行した場合は、ガイドではなく通常の新規状態が表示されます。
