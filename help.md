...
### 4. 点線切替ボタン「—, ‥」🆕
- 線を実線と点線で切り替えます（ボタンの表示は現在の線タイプを示します）
- 実線（—）：途切れのない連続した線
- 点線（‥）：点が連続する破線
- 選択中の線の線タイプを変更します
- 何も選択していない場合は、新規作成時のデフォルト線タイプを変更します
- ボタンがハイライト（青色）の場合は、点線が選択されていることを示します
...
### 2. 効率的な複数選択と一括編集
- ラバーバンド選択（ドラッグによる範囲選択）で複数のノードや線を選択
- 選択した要素に対して一括で設定変更が可能です
- 例：複数の線を一度に点線に変更したり、複数のノードを一括で太字にするなど
