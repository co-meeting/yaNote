...
- **囲み矩形**  
  空白部分での Shift＋左ドラッグ、またはコントロールパネルの▭ボタンから、ノードたちをかこう角丸・薄背景の矩形を描けるようになりました。矩形は常に最背面に表示され、枠線のドラッグで移動、四隅ハンドルでリサイズできます。

- **コントロールパネルの整理**  
  ボタンを「編集系｜表示系｜ファイル系」の3グループに区分し、新規・開く・保存・AI・共有の5ボタンをファイルメニュー（☰）に集約しました。

## 新機能 (v1.6 / 2026-07-18)

- **ズーム機能**  
  Ctrl/Cmd＋マウスホイール（トラックパッドのピンチ含む）とタッチの2本指ピンチで、カーソル位置（指の中間点）を中心に表示を拡大縮小できるようになりました（25%〜200%）。Ctrl/Cmd+0 で等倍に戻ります。ズーム率は保存され、次回起動時にも維持されます。

- **AI向けエクスポート機能**  
  コントロールパネルのAIボタンから、座標配置と接続の階層を解析した AI 向け Markdown を出力できるようになりました。エクスポート JSON にも書き手の使い分けを説明する凡例（`semantics`）が埋め込まれます。

## 改善 (v1.5 / 2026-07-18)

- **内部リファクタリング（ソースファイルの分離）**  
  1ファイルに収まっていた `index.html` から CSS と JavaScript をそれぞれ `style.css` / `app.js` に分離しました。ビルド不要・静的配信の構成はそのままで、機能・操作に変更はありません。

- **リリース作業のスクリプト化**  
  バージョン一括更新・リリースノート下書き生成・公開（commit / tag / GitHub Release）を `scripts/` のスクリプトで自動化しました（手順は [RELEASE.md](./RELEASE.md)）。

- **開発ガイドラインの刷新**  
  [yaNote Generative AI 開発ガイド](./docs/yaNote_DevGuide.md) をコーディングエージェント時代に合わせて更新し、リポジトリに `CLAUDE.md` を追加しました。

## 改善 (v1.4.2 / 2026-03-13)
...
- **クリップボードショートカットの拡張**  
  `Ctrl/Cmd+C`, `Ctrl/Cmd+V` に加えて `Ctrl/Cmd+X`（カット）に対応しました。

- **整列ボタン表示の改善**  
  コントロールパネルの整列ボタンを `▦` 表示に変更し、意味が伝わりやすいよう `aria-label` / `title` を追加しました。

## 改善 (v1.4.1 / 2026-03-11)
...
- **タッチ操作の本格対応**  
  スマートフォン・タブレットで、マウス操作と同等の操作が可能になりました。  
  - **パン**: シングルタッチ＋ドラッグでキャンバスを移動。  
...
- Shift＋クリックやラバーバンド選択で複数のノードや接続線を一括で選択・移動・削除できます。  
  さらに、Cmd/Ctrl+A キーの同時押しにより、キャンバス上の全オブジェクトを一括選択でき、同様の操作が可能です。  
   ...
   - Ctrl/Cmd＋マウスホイール（トラックパッドのピンチ、タッチでは2本指ピンチ）で表示を拡大縮小できます（25%〜200%）。Ctrl/Cmd+0 で等倍に戻ります。
...
5. **コントロールパネル**  
   - 画面右上のコントロールパネルから、ノード種類変更、線種・線タイプ変更、太字切替などが簡単に操作できます。新規作成、開く（インポート）、保存（エクスポート）、共有はファイルメニュー（☰）にまとまっています。  
   - 新規作成および開く操作時、未保存の状態がある場合は保存確認ダイアログが表示され、保存済みかの確認が求められます。

---

## その他

- **ベータ版について**: 従来のベータ環境（`beta/`）は 2026 年に廃止しました。新機能の検証は本番リポジトリ上で行い、本番のみの提供となります。
